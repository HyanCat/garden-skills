# 录制与后期合成（仅 Vite 互动 demo 模式）

> ⚠️ **本文档只在 Vite 模式下适用**。Remotion 模式（默认）直接
> `npx remotion render Full out/full.mp4` 出片，不需要屏幕录制。
> 详见 [`REMOTION.md`](REMOTION.md) 末尾"渲染命令"。
>
> 仅当用户**明确要现场可点击演示**才回退到本文档的录屏路径。
